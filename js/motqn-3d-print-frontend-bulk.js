...
function motqnGenerateAnalysisSignature(fileId, fileData) {
        ...
        if (fileData) {
                ...
                if (typeof fileData.dim_x !== 'undefined') {
                        signature.dim_x = fileData.dim_x;
                }
                if (typeof fileData.dim_y !== 'undefined') {
                        signature.dim_y = fileData.dim_y;
                }
                if (typeof fileData.dim_z !== 'undefined') {
                        signature.dim_z = fileData.dim_z;
                }
                if (typeof fileData.image_height !== 'undefined') {
                        signature.image_height = fileData.image_height;
                }
                if (typeof fileData.image_map !== 'undefined') {
                        signature.image_map = fileData.image_map;
                }
...
function motqnHandleStaleAnalysis(fileId, analysisToken) {
        ...
        if (typeof p3d !== 'undefined' && typeof p3d.analyse_queue !== 'undefined' && typeof p3d.analyse_queue[fileId] !== 'undefined') {
                ...
                delete p3d.analyse_queue[fileId].pending_analysis_signature;
        }
...
//console.log(JSON.stringify(custom_attributes));
		...
		.done(function( msg ) {
				...
				p3d.triangulated_volume = data.material_volume;
                                ...
                                var persistedSignature = fileData.pending_analysis_signature || motqnGenerateAnalysisSignature(file_id, fileData);
                                persistedSignature = motqnNormalizeStoredSignature(persistedSignature);

                                if (persistedSignature) {
                                        fileData.last_analysis_signature = persistedSignature;
                                }
                                else {
                                        delete fileData.last_analysis_signature;
                                }
			...
			bulk: true
                ...
                .done(function( msg ) {
                        ...
                        if (data.status=='1') {
                                ...
                                var persistedSignatureCheck = fileData.pending_analysis_signature || motqnGenerateAnalysisSignature(file_id, fileData);
                                persistedSignatureCheck = motqnNormalizeStoredSignature(persistedSignatureCheck);

                                if (persistedSignatureCheck) {
                                        fileData.last_analysis_signature = persistedSignatureCheck;
                                }
                                else {
                                        delete fileData.last_analysis_signature;
                                }
